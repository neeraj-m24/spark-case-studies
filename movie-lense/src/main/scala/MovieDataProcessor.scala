...
object MovieDataProcessor {
  def main(args: Array[String]): Unit = {
    ...
    parsedRatingsDF.writeStream.foreachBatch { (batchDF: Dataset[Row], batchId: Long) =>
        ...
        // Print parsedRatingsDF
        println("Parsed Ratings DataFrame:")
        ...
        // Print enrichedRatingsDF
        println("Enriched Ratings DataFrame:")
        enrichedRatingsDF.show(10, truncate = false)

        // Add a date column for partitioning
        val finalEnrichedDF = enrichedRatingsDF.withColumn("partitionDate", to_date(from_unixtime(col("timestamp") / 1000)))

        // Print finalEnrichedDF
        println("Final Enriched DataFrame:")
        finalEnrichedDF.show(10, truncate = false)
        ...
        // Print movieMetricsDF
        println("Movie Metrics DataFrame:")
        movieMetricsDF.show(10, truncate = false)

        movieMetricsDF.limit(100).write.mode("overwrite").parquet(movieMetricsPath)
        println("Movie metrics updated")
        ...
        // Print genreMetricsDF
        println("Genre Metrics DataFrame:")
        genreMetricsDF.show(10, truncate = false)

        genreMetricsDF.limit(100).write.mode("overwrite").parquet(genreMetricsPath)
        println("Genre metrics updated")
        ...
        // Print demographicMetricsDF
        println("Demographic Metrics DataFrame:")
        demographicMetricsDF.show(10, truncate = false)
